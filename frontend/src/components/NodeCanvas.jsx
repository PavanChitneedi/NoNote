...
export default function NodeCanvas({ mapId, onBack, onHome }) {
  ...
  const canvasRef_outer = useRef(null);

  useEffect(() => {
    const el = canvasRef_outer.current;
    if (!el) return;
    if (canvasTheme === "global") {
      // Remove all inline theme vars so parent vars apply
      Object.keys(THEMES.dark.vars).forEach(v => el.style.removeProperty(v));
    } else {
      const t = THEMES[canvasTheme];
      if (t) Object.entries(t.vars).forEach(([k,v]) => el.style.setProperty(k, v));
    }
  }, [canvasTheme, theme]);
  ...
  const scheduleSave = useCallback((ns, es) => {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        ...
        // Auto-snapshot every 5 min (debounced)
        clearTimeout(versionTimer.current);
        versionTimer.current = setTimeout(async () => {
          try {
            await saveVersion(mapId, { nodes:ns, edges:es, label:"Auto-save" });
          } catch {}
        }, 5 * 60 * 1000);
      } catch {
  ...
  useEffect(() => {
    ...
    const onUp=()=>{
      if (dragging||resizing) { setNodes(ns=>{setEdges(es=>{scheduleSave(ns,es);pushHistory(ns,es);return es;});return ns;}); }
  ...
  }, [dragging,resizing,drawingEdge,scheduleSave,pushHistory,zoom]);
  ...
  const updateNotes = (id, val) => {
    setNodes(ns=>ns.map(n=>n.id===id?{...n,notes:val}:n)); // instant UI
    clearTimeout(notesTimers.current[id]);
    notesTimers.current[id] = setTimeout(()=>{
      setNodes(ns=>{
        const updated=ns.map(n=>n.id===id?{...n,notes:val}:n);
        setEdges(es=>{ scheduleSave(updated,es); pushHistory(updated,es); return es; });
        return updated;
      });
    }, 800);
  };
  ...
  const handleRestore = (ns, es) => {
    ...
    const mappedEdges = es.map(e=>({
      ...
      label:e.label||"",style:e.style||"arrow",color:e.color||"var(--accent)",
    }));
    setNodes(mappedNodes); setEdges(mappedEdges);
    pushHistory(mappedNodes,mappedEdges);
    scheduleSave(mappedNodes,mappedEdges);
  };
  ...
  return (
    <div ref={canvasRef_outer} style={{display:"flex",flexDirection:"column",height:"100vh",background:"var(--bg)",overflow:"hidden"}}>
      ...
      <div style={{height:50,background:"var(--bg2)",borderBottom:"1px solid var(--border2)",display:"flex",alignItems:"center",gap:4,padding:"0 8px",flexShrink:0,overflowX:"auto"}}>
        ...
        <div style={{position:"relative",flexShrink:0}}>
          <button onClick={()=>setShowCanvasTheme(v=>!v)} style={tbtn(canvasTheme!=="global","#6C63FF")} title="Canvas theme">
            {canvasTheme==="global"?THEMES[themeName]?.icon||"🌐":"🎨"} THEME
          </button>
          {showCanvasTheme&&(
            <div style={{position:"absolute",top:"calc(100% + 6px)",right:0,background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:10,padding:8,zIndex:100,minWidth:180,boxShadow:"0 8px 24px var(--shadow)"}}>
              {CANVAS_THEMES.map(t=>(
                <div key={t.id} onClick={()=>{setCanvasTheme(t.id);setShowCanvasTheme(false);}}
                  style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",borderRadius:7,cursor:"pointer",background:canvasTheme===t.id?"var(--accent2)20":"transparent",fontSize:12,color:canvasTheme===t.id?"var(--accent)":"var(--text)"}}>
                  <span>{t.icon}</span>
                  <span>{t.label}</span>
                  {canvasTheme===t.id&&<span style={{marginLeft:"auto",color:"var(--accent)"}}>✓</span>}
                </div>
              ))}
            </div>
          )}
      ...
      {/* Close canvas theme picker on outside click */}
      {showCanvasTheme&&<div style={{position:"fixed",inset:0,zIndex:99}} onClick={()=>setShowCanvasTheme(false)}/>}
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden",position:"relative"}}>
        ...
        {/* ── Canvas ── */}
        <div ref={canvasRef}
          onClick={()=>{setSelected(null);setSelType(null);setShowCanvasTheme(false);if(drawingEdge)setDrawingEdge(null);}}
          onMouseMove={e=>{
            if(drawingEdge&&canvasRef.current){
              const rect=canvasRef.current.getBoundingClientRect(); const s=1/zoom;
